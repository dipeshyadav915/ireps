...
async function fetchTender(date) {
  ...
  formData.append('tender_type', 'archive');
...
export async function GET(req) {
  ...
  try {
    for (const tenderData of data) {
      const ifExistArchive = await ArchiveMainTender.findOne({
        where: {
          fld_id: tenderData.fld_id,
        },
      });
      if (!ifExistArchive) {
        await ArchiveMainTender.create({
          fld_id: tenderData.fld_id,
        ...
        if (tenderData.documents) {
          await Promise.all(
            tenderData.documents.map(async (doc) => {
              await ArchiveDocs.create({
                tg_main_tender_id: doc.tg_main_tender_id,
                file_doc_type: doc.file_doc_type,
                file_doc_name: doc.file_doc_name,
                file_doc_description: doc.file_doc_description,
                file_doc_path: doc.file_doc_path,
                status: doc.status,
        ...
        if (tenderData.sectors) {
          await Promise.all(
            tenderData.sectors.map(async (sector) => {
              await ArchiveTenderSectors.create({
                tg_main_tender_id: sector.tg_main_tender_id,
                sector_id: sector.sector_id,
                status: sector.status,
                created: sector.created,
                modified_date: sector.modified_date,
              });
            }),
          );
        }
        ...
        const isExistArchive = await MainTender.findOne({
          where: { fld_id: tenderData.fld_id },
        });

        if (isExistArchive) {
          await MainTender.destroy({
            where: { fld_id: tenderData.fld_id },
          });

          await TenderSectors.destroy({
            where: { tg_main_tender_id: tenderData.fld_id },
          });

          await TenderDocs.destroy({
            where: { tg_main_tender_id: tenderData.fld_id },
          });
        }
      }
